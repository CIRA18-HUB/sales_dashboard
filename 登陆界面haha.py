...
def update_dynamic_stats():
    ...
    if time_elapsed >= 5:
        ...
        # 分析模块 - 稳定变化
        st.session_state.stat2_value = max(1, 4 + random.randint(-1, 1))

        # 小时监控 - 周期性变化
        st.session_state.stat3_value = max(1, 24 + int(math.sin(current_time * 0.2) * 8))

        # 准确率 - 波动变化
        st.session_state.stat4_value = max(85,
                                           min(99, 95 + random.randint(0, 4) + int(math.sin(current_time * 0.15) * 3)))
