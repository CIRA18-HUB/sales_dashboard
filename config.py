...
DATA_FILES = {
    ...
    # 库存分析专用文件
    'batch_inventory': "含批次库存.xlsx",  # 批次库存数据
    'shipping_data': "出货数据.xlsx",  # 出货数据
    'inventory_forecast': "人工预测.xlsx",  # 库存预测数据
    'price_data': "单价.xlsx"  # 产品单价
}
...
def load_inventory_data():
    ...
    try:
        ...
        # 加载批次库存数据
        if os.path.exists(DATA_FILES['batch_inventory']):
            inventory_raw = pd.read_excel(DATA_FILES['batch_inventory'])
        ...
        # 加载出货数据
        if os.path.exists(DATA_FILES['shipping_data']):
            shipping_data = pd.read_excel(DATA_FILES['shipping_data'])
        ...
        # 加载预测数据
        if os.path.exists(DATA_FILES['inventory_forecast']):
            forecast_data = pd.read_excel(DATA_FILES['inventory_forecast'])
        ...
        # 加载单价数据
        if os.path.exists(DATA_FILES['price_data']):
            price_df = pd.read_excel(DATA_FILES['price_data'])
            price_dict = {}
