...
DATA_FILES = {
    ...
    # 库存分析专用文件
    'batch_inventory': os.path.join(ROOT_DIR, "含批次库存.xlsx"),  # 批次库存数据
    ...
    try:
        ...
        if os.path.exists(shipping_data_path):
            try:
                ...
                # 判断文件结构以决定如何处理
                if '订单日期' in shipping_data.columns:
                    # 文件已有正确的列名
                    pass
                elif '发运月份' in shipping_data.columns:
                    # 重命名列名以适应当前代码
                    shipping_data = shipping_data.rename(columns={
                        '发运月份': '订单日期',
                        '所属区域': '所属区域',
                        '申请人': '申请人',
                        '产品代码': '产品代码',
                    ...
                    # 假设前5列是我们需要的数据
                    if len(shipping_data.columns) >= 5:
                        shipping_data.columns = ['订单日期', '所属区域', '申请人', '产品代码', '数量'] + list(
                            shipping_data.columns[5:])
        ...
        if os.path.exists(forecast_data_path):
            try:
                ...
                # 判断文件结构
                if '所属大区' in forecast_data.columns and '预计销售量' in forecast_data.columns:
                    # 已有正确列名
                    pass
                elif '所属区域' in forecast_data.columns:
                    # 需要重命名列
                    forecast_data = forecast_data.rename(columns={'所属区域': '所属大区'})
                else:
                    # 假设列结构
                    if len(forecast_data.columns) >= 5:
                        forecast_data.columns = ['所属大区', '销售员', '所属年月', '产品代码', '预计销售量'] + list(
                            forecast_data.columns[5:])
