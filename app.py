...
from utils.data_loader import load_all_data, load_page_data
...
# 初始化会话状态
initialize_session_state('authenticated', False)
...
def main():
    ...
    # 主应用页面
        ...
        # 加载基础数据（如果尚未加载）
        if not st.session_state.base_data_loaded:
            with st.spinner("正在加载基础数据..."):
                # 只加载核心数据
                basic_data_types = ['sales_data', 'product_codes', 'new_product_codes']
                st.session_state.data = load_all_data(DATA_PATHS, basic_data_types)
        ...
        with st.sidebar:
            ...
            if 'sales_data' in st.session_state.data and not st.session_state.data['sales_data'].empty:
                # 区域筛选
