...
if __name__ == "__main__" and 'authenticated' not in st.session_state:
    # 初始化认证状态为未登录
...
initialize_session_state('demo_mode', False)
...
def main():
    ...
    # 在用户登录后再转换数据文件，并添加进度指示器
    with st.spinner("正在优化数据格式，首次加载可能需要几分钟..."):
        global DATA_PATHS
    ...
    with st.sidebar:
        ...
        end_of_year = pd.Timestamp(f"{current_year}-12-31")

        # 如果session_state中已有date_range，则使用已有的值作为默认值
        default_dates = st.session_state.filters.get('date_range', [start_of_year, end_of_year])
        if default_dates is None:
            default_dates = [start_of_year, end_of_year]

        # 设置日期范围筛选器
        date_range = st.date_input(
            "选择时间范围",
            default_dates,
            min_value=pd.Timestamp("2020-01-01"),  # 设置一个合理的最小日期
            max_value=pd.Timestamp("2030-12-31"),  # 设置一个合理的最大日期
            format="YYYY/MM/DD"  # 使用Streamlit支持的日期格式
        )
        ...
        # 如果sales_data已加载，创建筛选器
            ...
            if st.button("重置筛选器"):
                st.session_state.filters = {
                    ...
                    'customer': None,
                    'product': None,
                    'date_range': [start_of_year, end_of_year]  # 重置为当年
                }
                st.rerun()
