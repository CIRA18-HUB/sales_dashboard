...
# 自定义CSS样式 - 完全采用sales_dashboard.py的样式
st.markdown("""
<style>
    .main-header {
        font-size: 2rem;
    ...
    /* 重置按钮 */
    button.stButton {
        background-color: #1f3867 !important;
    ...
    button.stButton:hover {
        ...
        box-shadow: 0 0.15rem 0.5rem rgba(31, 56, 103, 0.3) !important;
    }
...
# 美化的侧边栏标题
st.sidebar.markdown("""
<h3 style="color: #1f3867; margin-bottom: 1.5rem; text-align: center; padding-bottom: 0.75rem; border-bottom: 1px solid #e5e7eb;">
    🔍 数据筛选
</h3>
""", unsafe_allow_html=True)
...
if "filter_salesperson" not in st.session_state:
    st.session_state.filter_salesperson = "全部"
...
selected_salesperson = st.sidebar.selectbox(
    ...
    all_salespersons,
    index=0,
    key="sidebar_salesperson"
)
st.session_state.filter_salesperson = selected_salesperson
...
# 日期范围筛选器
st.sidebar.markdown("<h4 style='color: #1f3867;'>日期范围</h4>", unsafe_allow_html=True)
...
# 重置筛选按钮 - 更美观的样式
st.sidebar.markdown("<div style='padding: 1rem 0;'></div>", unsafe_allow_html=True)
if st.sidebar.button("重置筛选条件", key="reset_filters"):
    st.session_state.filter_region = "全部"
    st.session_state.filter_salesperson = "全部"
    st.session_state.filter_customer = "全部"
    st.session_state.filter_date_range = (
...
# 应用区域筛选
if st.session_state.filter_region != "全部":
    filtered_data = filtered_data[filtered_data['所属区域'] == st.session_state.filter_region]

# 应用销售员筛选
if st.session_state.filter_salesperson != "全部":
    filtered_data = filtered_data[filtered_data['申请人'] == st.session_state.filter_salesperson]

# 应用客户筛选
if st.session_state.filter_customer != "全部":
    filtered_data = filtered_data[filtered_data['客户简称'] == st.session_state.filter_customer]
...
def analyze_customer_data(data):
    ...
    customer_sales = data.groupby(['客户代码', '客户简称']).agg({
        '销售额': 'sum',
        '产品代码': 'nunique',
        '申请人': lambda x: x.value_counts().index[0]
    }).reset_index()
...
if filtered_data.empty:
    ...
    with col2:
        concentration_color = '#4CAF50' if top5_concentration <= 50 else '#FF9800' if top5_concentration <= 70 else '#F44336'
        st.markdown(f"""
        <div class="metric-card">
    ...
    with col3:
        ...
        <div class="metric-card">
            ...
            <p class="card-value">{format_yuan(avg_customer_value)}</p>
        ...
        if not customer_sales.empty:
            ...
            if fig:
                ...
                if len(customer_sales) > 0:
                    ...
                    st.markdown(f"""
                    <div class="chart-explanation">
                        <b>图表解读：</b> {top1_name}是最大客户，销售额{format_yuan(top1_sales)}，占总销售额的{format_percentage(top1_percentage)}。
            ...
            for i, row in top5_customers.iterrows():
                ...
                customer_percentage = (customer_sales_value / customer_sales['销售额'].sum() * 100) if customer_sales[
                                                                                                           '销售额'].sum() > 0 else 0
                ...
                <div style="background-color: white; padding: 1.5rem; border-radius: 0.5rem; 
                    ...
                    <div style="display: flex; flex-wrap: wrap;">
                        <div style="flex: 1; min-width: 200px; margin-right: 1rem;">
                            <p><strong>销售额：</strong> {format_yuan(customer_sales_value)}</p>
            ...
            with cols[1]:
                ...
                if fig:
                    ...
                    if len(region_stats) > 0:
                        ...
                        st.markdown(f"""
                        <div class="chart-explanation">
                            <b>图表解读：</b> {highest_value_region}区域平均客户价值最高，为{format_yuan(highest_avg_value)}。
    ...
    with tabs[3]:  # 客户价值分析
        ...
        if not customer_sales.empty:
            ...
            if fig:
                ...
                st.markdown(f"""
                <div class="chart-explanation">
                    <b>图表解读：</b> 此图展示了客户销售额与产品多样性的关系。平均客户价值为{format_yuan(avg_value)}，平均购买产品种类为{avg_variety:.1f}种。
            ...
            if '客户类型' in customer_sales.columns:
                ...
                if 'Sales ID' not in customer_segments.columns:
                    customer_segments['客户数量'] = customer_sales.groupby('客户类型').size().values
                else:
                    customer_segments.rename(columns={'Sales ID': '客户数量'}, inplace=True)
                ...
                with col1:
                    ...
                    if not core_data.empty:
                        core_count = core_data.iloc[0]['客户数量']
                        core_sales = core_data.iloc[0]['销售额']
                        core_count_pct = core_data.iloc[0]['客户占比']
                        core_sales_pct = core_data.iloc[0]['销售额占比']
                        ...
                        <div style="background-color: rgba(76, 175, 80, 0.1); border-left: 4px solid #4CAF50; 
                            ...
                            <p><b>销售贡献：</b> {format_yuan(core_sales)} ({format_percentage(core_sales_pct)})</p>
                    ...
                    if not diverse_data.empty:
                        diverse_count = diverse_data.iloc[0]['客户数量']
                        diverse_sales = diverse_data.iloc[0]['销售额']
                        diverse_count_pct = diverse_data.iloc[0]['客户占比']
                        diverse_sales_pct = diverse_data.iloc[0]['销售额占比']
                        ...
                        <div style="background-color: rgba(255, 152, 0, 0.1); border-left: 4px solid #FF9800; 
                            ...
                            <p><b>销售贡献：</b> {format_yuan(diverse_sales)} ({format_percentage(diverse_sales_pct)})</p>
                ...
                with col2:
                    ...
                    if not single_data.empty:
                        single_count = single_data.iloc[0]['客户数量']
                        single_sales = single_data.iloc[0]['销售额']
                        single_count_pct = single_data.iloc[0]['客户占比']
                        single_sales_pct = single_data.iloc[0]['销售额占比']
                        ...
                        <div style="background-color: rgba(33, 150, 243, 0.1); border-left: 4px solid #2196F3; 
                            ...
                            <p><b>销售贡献：</b> {format_yuan(single_sales)} ({format_percentage(single_sales_pct)})</p>
                    ...
                    if not marginal_data.empty:
                        marginal_count = marginal_data.iloc[0]['客户数量']
                        marginal_sales = marginal_data.iloc[0]['销售额']
                        marginal_count_pct = marginal_data.iloc[0]['客户占比']
                        marginal_sales_pct = marginal_data.iloc[0]['销售额占比']
                        ...
                        <div style="background-color: rgba(244, 67, 54, 0.1); border-left: 4px solid #F44336; 
                            ...
                            <p><b>销售贡献：</b> {format_yuan(marginal_sales)} ({format_percentage(marginal_sales_pct)})</p>
    ...
    st.markdown(f"""
    <div style="background-color: rgba(31, 56, 103, 0.1); border-left: 4px solid #1f3867; 
                padding: 1rem; border-radius: 0.5rem;">
        <h4>📋 客户分析总结</h4>
        <p><strong>客户基础：</strong>当前共有{format_number(total_customers)}个活跃客户，平均客户价值{format_yuan(avg_customer_value)}。</p>
