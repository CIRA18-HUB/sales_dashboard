...
# 自定义CSS样式 - 使用与sales_dashboard.py完全相同的样式
st.markdown("""
<style>
    .main-header {
        font-size: 2rem;
    ...
    with tabs[3]:  # 客户价值分析
        ...
        if not customer_sales.empty:
            ...
            if '客户类型' in customer_sales.columns:
                ...
                # 重命名列
                customer_segments = customer_segments.rename(columns={'customer_count': '客户数量'})
                if '客户数量' not in customer_segments.columns:
