...
if not st.session_state.authenticated:
    st.markdown(
        '<div style="font-size: 1.8rem; color: #1E88E5; text-align: center; margin-bottom: 1.5rem; font-weight: 600;">客户分析仪表盘 | 登录</div>',
        unsafe_allow_html=True)
    ...
    with col2:
        st.markdown("""
        <div class="card" style="max-width: 450px; margin: 2rem auto; padding: 2rem;">
            <h2 style="text-align: center; color: #1E88E5; margin-bottom: 1.5rem; font-size: 1.8rem; font-weight: 600;">请输入密码</h2>
        </div>
        """, unsafe_allow_html=True)
...
def create_customer_filters(data):
    ...
    with st.sidebar:
        ...
        # 1. 区域筛选 - 修复默认选择"全部"的问题
        if '所属区域' in orders.columns:
            regions = sorted(list(orders['所属区域'].unique()))
            all_regions = ['全部'] + regions
    ...
    try:
        ...
        if not customer_target.empty and '月度指标' in customer_target.columns:
            try:
                ...
                # 如果结果为空，尝试使用更宽松的合并策略
                if merged.empty and not target_data.empty:
                    # 清理客户名称（去除空格、统一大小写）
                    sales_monthly['客户名称_清理'] = sales_monthly[customer_col].str.strip().str.lower()
                    target_data['客户名称_清理'] = target_data['客户'].str.strip().str.lower()

                    # 使用清理后的名称合并
                    merged = pd.merge(sales_monthly, target_data,
                                      left_on=['客户名称_清理', '月份'],
                                      right_on=['客户名称_清理', '月份'], how='inner')
...
def create_target_achievement_chart(data):
    ...
    try:
        ...
        # 如果结果为空，尝试更宽松的匹配方式
        if merged.empty and not customer_target.empty:
            # 清理客户名称（去除空格、统一大小写）
            sales_monthly['客户名称_清理'] = sales_monthly[customer_col].str.strip().str.lower()
            customer_target_clean = customer_target.copy()
            customer_target_clean['客户名称_清理'] = customer_target_clean['客户'].str.strip().str.lower()

            # 使用清理后的名称合并
            merged = pd.merge(sales_monthly, customer_target_clean,
                              left_on=['客户名称_清理', '月份'],
                              right_on=['客户名称_清理', '月份'], how='inner')
...
def create_customer_product_preference(data):
    ...
    try:
        ...
        # 只保留有意义的产品（降低阈值至2%）
        significant_products = pivot_pct.columns[pivot_pct.max() >= 2]

        # 如果满足条件的产品太少，至少保留前5个主要产品
        if len(significant_products) < 5 and len(pivot_pct.columns) > 0:
            product_max_values = pivot_pct.max().sort_values(ascending=False)
            significant_products = product_max_values.index[:min(5, len(product_max_values))]
...
def create_customer_trend_chart(data):
    ...
    try:
        ...
        monthly_stats['客户环比'] = monthly_stats['客户环比'].fillna(0)  # 填充首月的NaN值

        monthly_stats['销售额环比'] = monthly_stats['销售额'].pct_change() * 100
        monthly_stats['销售额环比'] = monthly_stats['销售额环比'].fillna(0)  # 填充首月的NaN值
...
def create_region_dependency_risk_chart(data):
    ...
    try:
        ...
        # 计算每个区域的总销售额
        region_total_sales = orders.groupby('所属区域')['销售额'].sum().reset_index()
        region_total_sales.columns = ['所属区域', '区域总销售额']

        # 合并区域总销售额到客户销售额数据
        region_customer_sales = pd.merge(region_customer_sales, region_total_sales, on='所属区域')

        # 计算每个客户在其所在区域的依赖度
        region_customer_sales['依赖度'] = region_customer_sales['销售额'] / region_customer_sales['区域总销售额'] * 100

        # 找出每个区域依赖度最高的客户索引
        max_dependency_indices = region_customer_sales.groupby('所属区域')['依赖度'].idxmax()

        # 筛选出依赖度最高的客户行
        max_dependency_by_region = region_customer_sales.loc[max_dependency_indices].copy()
        ...
        max_dependency_by_region[['风险等级', '风险颜色']] = max_dependency_by_region['依赖度'].apply(
        ...
        if '客户简称' in orders.columns:
            customer_name_map = orders.drop_duplicates([customer_col, '客户简称']).set_index(customer_col)[
                '客户简称'].to_dict()
            max_dependency_by_region['客户简称'] = max_dependency_by_region[customer_col].map(
                lambda x: customer_name_map.get(x, x)
            )
        else:
            max_dependency_by_region['客户简称'] = max_dependency_by_region[customer_col]
        ...
        for idx, row in max_dependency_by_region.iterrows():
            fig.add_trace(go.Scatter(
                ...
                hovertemplate=f'<b>{row["所属区域"]}</b><br>' +
                              ...
                              f'依赖度: {row["依赖度"]:.1f}%<br>' +
                              f'客户销售额: {format_currency(row["销售额"])}<br>' +
        ...
        fig.update_layout(
            ...
            xaxis_title="区域",
            yaxis_title="最大客户依赖度 (%)",
            yaxis=dict(range=[0, max(max_dependency_by_region['依赖度'].max() * 1.1, 50)]),
            height=500,
            plot_bgcolor='white'
        )
...
with tabs[0]:
    # 核心指标卡片
    st.markdown('<div class="sub-header">🔑 核心客户指标</div>', unsafe_allow_html=True)
    ...
    # 重点客户销售排行
    st.markdown('<div class="sub-header">📊 重点客户销售贡献TOP20</div>', unsafe_allow_html=True)
...
# 目标达成分析标签
with tabs[1]:
    st.markdown('<div class="sub-header">🎯 重点客户目标达成分析</div>', unsafe_allow_html=True)
...
with tabs[2]:
    # 客户趋势分析
    st.markdown('<div class="sub-header">📈 客户数量与销售趋势</div>', unsafe_allow_html=True)
    ...
    # 客户产品偏好分析
    st.markdown('<div class="sub-header">🛒 重点客户产品偏好强度</div>', unsafe_allow_html=True)
...
# 风险分析标签
with tabs[3]:
    st.markdown('<div class="sub-header">⚠️ 区域客户依赖风险分析</div>', unsafe_allow_html=True)
