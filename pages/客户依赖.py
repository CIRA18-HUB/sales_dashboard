            ...
            for i, row in customer_sales.iterrows():
                ...
                if '产品代码' in customer_orders.columns and '产品简称' in customer_orders.columns:
                    ...
                    for _, prod in product_sales.iterrows():
                        top_products.append(f"{prod['产品简称']}: {config.format_currency(prod['销售额'])}")
...
# 创建页面内的标签 - 类似预测与计划.py
tabs = st.tabs(["📊 客户概览", "🔍 客户详情分析", "📈 客户趋势", "🌐 区域分析"])
...
with tabs[0]:
    ...
    with col1:
        ...
        <div class="metric-card">
            ...
            <p class="card-value">{config.format_number(kpis.get('total_customers', 0))}</p>
    ...
    with col2:
        ...
        <div class="metric-card">
            ...
            <p class="card-value">{config.format_currency(kpis.get('avg_customer_sales', 0))}</p>
    ...
    with col4:
        ...
        <div class="metric-card">
            ...
            <p class="card-value">{config.format_percentage(kpis.get('target_achievement_rate', 0))}</p>
