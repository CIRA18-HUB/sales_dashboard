            ...
            for i, row in customer_sales.iterrows():
                ...
                if '产品代码' in customer_orders.columns:
                    ...
                    for _, prod in product_sales.iterrows():
                        top_products.append(f"{prod[product_name_col]}: {config.format_currency(prod['销售额'])}")

                info = (
                    f"销售额: {config.format_currency(row['销售额'])}<br>"
                    f"订单数: {order_count}<br>"
                    f"平均订单金额: {config.format_currency(avg_order_value)}<br>"
    ...
    try:
        ...
        for _, row in region_stats.iterrows():
            hover_text.append(
                ...
                f"销售额: {config.format_currency(row['销售额'])} ({row['销售额占比']:.1f}%)<br>" +
                f"客户均销售额: {config.format_currency(row['客户均销售额'])}<br>" +
                f"客户均订单数: {row['客户均订单数']:.1f}<br>" +
                f"订单均销售额: {config.format_currency(row['订单均销售额'])}"
            )
            ...
            for j, product in enumerate(pivot_pct.columns):
                ...
                if pct_value > 0:
                    customer_hover.append(
                        f"客户: {customer}<br>" +
                        f"产品: {product}<br>" +
                        f"销售额: {config.format_currency(actual_value)}<br>" +
                        f"占比: {pct_value:.1f}%"
                    )
                else:
                    customer_hover.append("")
...
# 主程序部分
# 加载数据 - 使用config.py中的函数
data = config.load_data_files()
...
with tabs[0]:
    ...
    with col1:
        ...
        st.markdown(
            create_clickable_metric_card(
                header="客户总数",
                value=config.format_number(kpis.get('total_customers', 0)),
    ...
    with col3:
        ...
        st.markdown(
            create_clickable_metric_card(
                header="目标达成率",
                value=config.format_percentage(kpis.get('target_achievement_rate', 0)),
    ...
    with col4:
        ...
        st.markdown(
            create_clickable_metric_card(
                header="客户均销售额",
                value=config.format_currency(kpis.get('avg_customer_sales', 0)),
                description="平均每客户贡献",
