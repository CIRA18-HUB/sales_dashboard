...
def create_high_risk_priority_chart(batch_analysis):
    ...
    for i in range(len(high_risk)):
        clearance_days = '无法预计' if high_risk.iloc[i][
                                           '预计清库天数'] == 999 else f"{high_risk.iloc[i]['预计清库天数']:.0f}天"
        recommendation = high_risk.iloc[i]['建议措施']
        combined_customdata.append([clearance_days, recommendation])
    ...
    fig = go.Figure(data=go.Scatter(
        ...
        hovertemplate='<b>%{text}</b><br>' +
                      '库龄: %{x}天<br>' +
                      '批次价值: ¥%{y:,.0f}<br>' +
                      '清库天数: %{customdata[0]}<br>' +
                      '建议措施: %{customdata[1]}<extra></extra>',
