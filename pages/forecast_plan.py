...
DATA_FILES = {
    'actual_data': os.path.join(ROOT_DIR, "2409~250224出货数据.xlsx"),
    'forecast_data': os.path.join(ROOT_DIR, "2409~2502人工预测.xlsx"),
    'product_info': os.path.join(ROOT_DIR, "产品信息.xlsx")
}
...
def load_product_info(file_path=None):
    ...
    try:
        ...
        if file_path is None or not os.path.exists(file_path):
            file_path = DATA_FILES['product_info']
...
current_year_months = [f"{current_year}-{month:02d}" for month in range(1, 13)]

# 筛选共有月份数据
common_months = get_common_months(actual_data, forecast_data)

# 默认选择当年的共有月份
default_months = [month for month in common_months if month.startswith(str(current_year))]
if not default_months and common_months:  # 如果当年没有数据，则使用最近的月份
    default_months = [common_months[-1]]
...
with st.sidebar:
    ...
    # 添加重置筛选器按钮
    if st.button("重置筛选条件"):
        # 不直接修改st.session_state，使用rerun()方式
        st.rerun()  # 这会重置所有组件
    ...
    if not selected_months or not selected_regions:
        ...
        if selected_region_for_diff == '全国':
            ...
            if analysis_dimension == '产品':
                diff_data = processed_data['merged_monthly'].groupby(['产品代码', '所属区域']).agg({
                    '求和项:数量（箱）': 'sum',
                    '预计销售量': 'sum',
                }).reset_index()

                # 合并销售员信息(按区域和产品分组)
                sales_info = processed_data['merged_by_salesperson'].groupby(['所属区域', '产品代码', '销售员']).agg({
                    '求和项:数量（箱）': 'sum'
                }).reset_index()
                ...
                diff_data = processed_data['merged_by_salesperson'].groupby(['销售员', '所属区域', '产品代码']).agg({
                    '求和项:数量（箱）': 'sum',
                    '预计销售量': 'sum'
                }).reset_index()
            ...
            # 选定区域数据，按选定维度汇总
            region_filtered = processed_data['merged_monthly'][processed_data['merged_monthly']['所属区域'] == selected_region_for_diff]
            region_filtered_salesperson = processed_data['merged_by_salesperson'][
                processed_data['merged_by_salesperson']['所属区域'] == selected_region_for_diff]
        ...
        # 处理产品名称显示
        if analysis_dimension == '产品':
            diff_summary['产品名称'] = diff_summary['产品代码'].apply(lambda x: product_info[product_info['产品代码'] == x]['产品名称'].iloc[0] if len(product_info[product_info['产品代码'] == x]) > 0 else '')
        ...
        for idx, row in top_diff_items.iterrows():
            if analysis_dimension == '产品':
                ...
                if selected_region_for_diff == '全国':
                    # 查找该产品在所有选定月份的数据
                    product_details = processed_data['merged_monthly'][processed_data['merged_monthly']['产品代码'] == row['产品代码']]
                    ...
                    for region, region_data in product_details.groupby('所属区域'):
                        ...
                        # 找出该区域主要销售员
                        region_salesperson = processed_data['merged_by_salesperson'][
                            (processed_data['merged_by_salesperson']['产品代码'] == row['产品代码']) &
                            (processed_data['merged_by_salesperson']['所属区域'] == region)
                            ]
        ...
        if selected_scope == "全国":
            ...
            if not processed_data['national_top_skus'].empty:
                ...
                national_top_skus['产品名称'] = national_top_skus['产品代码'].apply(
                    lambda x: product_info[product_info['产品代码'] == x]['产品名称'].iloc[0] if len(product_info[product_info['产品代码'] == x]) > 0 else ''
                )
                ...
                try:
                    ...
                    product_growth_data = calculate_product_growth(
                        actual_monthly=filtered_actual,
                ...
                # 获取所选区域的重点SKU数据
                if selected_scope in processed_data['regional_top_skus'] and not processed_data['regional_top_skus'][
                    selected_scope].empty:
                    region_top = processed_data['regional_top_skus'][selected_scope].copy()
                    ...
                    region_top['产品名称'] = region_top['产品代码'].apply(
                        lambda x: product_info[product_info['产品代码'] == x]['产品名称'].iloc[0] if len(
                            product_info[product_info['产品代码'] == x]) > 0 else ''
                    )
            ...
            if selected_scope != "全国":  # 只有在选择特定区域时才显示对比分析
                ...
                # 获取区域和全国的SKU列表
                region_top = processed_data['regional_top_skus'][selected_scope] if selected_scope in processed_data[
                    'regional_top_skus'] else pd.DataFrame()
                if not region_top.empty and not processed_data['national_top_skus'].empty:
                    region_skus = set(region_top['产品代码'])
                    national_skus = set(processed_data['national_top_skus']['产品代码'])
                    ...
                    # 创建区域和全国重点SKU的名称映射
                    common_sku_names = [format_product_code(code, product_info, include_name=True) for code in
                                        common_skus]
                    ...
                    hover_texts = [
                        ...
                        '<br>- '.join(
                            [''] + [format_product_code(code, product_info, include_name=True) for code in
                                    common_skus]),
