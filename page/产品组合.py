...
# 星品&新品年度KPI考核产品代码
STAR_NEW_PRODUCTS = ['F3409N', 'F3406B', 'F01E6B', 'F01D6B', 'F01D6C', 'F01K7A']
...
# 数据文件路径
DATA_FILES = {
    'sales_data': r"C:\Users\何晴雅\Desktop\仪表盘原始数据.xlsx",
    'promotion_data': r"C:\Users\何晴雅\Desktop\仪表盘促销活动.xlsx",
    'star_new_products': r"C:\Users\何晴雅\Desktop\星品&新品年度KPI考核.txt"
}
...
def create_sample_sales_data():
    ...
    data = {
        ...
        '产品代码': (NEW_PRODUCTS + STAR_NEW_PRODUCTS + ['F0104L', 'F01E4B']) * 4,
        '产品名称': ['示例产品' + str(i) for i in range(40)],
        '订单类型': ['订单-正常产品'] * 40,
        '单价（箱）': [100 + i * 10 for i in range(40)],
        '数量（箱）': [10 + i for i in range(40)],
    }
...
def create_sample_promotion_data():
    ...
    data = {
        ...
        '经销商名称': ['客户A', '客户B', '客户C'] * 5,
        '产品代码': (NEW_PRODUCTS * 3),
        '促销产品名称': ['促销产品' + str(i) for i in range(15)],
        '预计销量（箱）': [100 + i * 10 for i in range(15)],
        '预计销售额（元）': [10000 + i * 1000 for i in range(15)],
        '促销开始供货时间': ['2025-04-01'] * 15,
        '促销结束供货时间': ['2025-04-30'] * 15
    }
...
def calculate_bcg_metrics(sales_data):
    """计算BCG产品组合指标"""
    if sales_data.empty:
        return {}
    ...
    cash_cow_percent = bcg_summary.loc[bcg_summary['BCG分类'] == '现金牛产品', '销售占比'].sum() if '现金牛产品' in \
                                                                                                    bcg_summary[
                                                                                                        'BCG分类'].values else 0
    star_question_percent = bcg_summary.loc[bcg_summary['BCG分类'].isin(['明星产品', '问号产品']), '销售占比'].sum()
...
def create_kpi_progress_chart(current_ratio, target_ratio):
    """创建KPI达成进度图表"""
...
def main():
    ...
    bcg_analysis = calculate_bcg_metrics(filtered_sales)
    ...
    with tabs[1]:  # 产品发展分析
        ...
        if not filtered_sales.empty:
            ...
            if not star_new_products_detail.empty:
                product_performance = star_new_products_detail.groupby(['产品代码', '简化产品名称']).agg({
                    '销售额': 'sum',
                    '数量（箱）': 'sum'
    ...
    with tabs[3]:  # 新品分析
        ...
        if not new_products_data.empty:
            ...
            # 新品详细表现
            new_product_performance = new_products_data.groupby(['产品代码', '简化产品名称']).agg({
                '销售额': 'sum',
                '数量（箱）': 'sum',
                '客户简称': 'nunique'
            }).reset_index().sort_values('销售额', ascending=False)
        ...
        if not sales_overview.get('region_sales', pd.Series()).empty:
            ...
            for i, (col, (_, row)) in enumerate(zip([col1, col2, col3], region_df.head(3).iterrows())):
                ...
                with col:
                    st.markdown(f"""
                    <div class="metric-card">
                        <p class="card-header">{rank_color} 第{row['排名']}名</p>
                        <p class="card-value">{row['区域']}</p>
                        <p class="card-text">{format_currency(row['销售额'])} ({format_percentage(row['占比'])})</p>
                    </div>
                    """, unsafe_allow_html=True)
