...
def load_all_data(data_paths, required_data_types=None):
    ...
    # 基础数据（一定会加载的）
    base_data_types = ['sales_data', 'product_codes', 'new_product_codes']
    ...
    if required_data_types is None:
        ...
        # 确保基础数据被加载
        data_types_to_load = list(set(base_data_types + required_data_types))
...
def load_page_data(data_paths, page_name, current_data=None):
    ...
    if missing_data_types:
        new_data = load_all_data(data_paths, missing_data_types)
        # 合并到当前数据
