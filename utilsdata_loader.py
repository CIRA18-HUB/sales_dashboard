...
# 检查 load_data_file 是否正确导入
from utils.data_loader import load_data_file

# 缓存单个数据文件加载
# 缓存单个数据文件加载
@st.cache_data(ttl=604800)  # 缓存一周 (7*24*60*60=604800秒)
def load_data_file(file_path, file_type=None):
    """
    缓存加载单个数据文件，优先使用parquet格式

    参数:
        file_path (str): 文件路径
        file_type (str): 文件类型，用于特殊处理

    返回:
        DataFrame/list: 加载的数据
    """
    try:
        # 检查是否有对应的parquet文件
        ...
        # 如果存在parquet文件，优先使用
        if os.path.exists(parquet_path) and file_path.endswith('.xlsx'):
        ...
        if file_path.endswith('.parquet'):
            ...
            # 特殊处理日期列(和Excel处理一致)
            if file_type == 'sales_data' and '发运月份' in data.columns:
                data['发运月份'] = pd.to_datetime(data['发运月份'])
            elif file_type == 'monthly_inventory' and '所属年月' in data.columns:
                data['所属年月'] = pd.to_datetime(data['所属年月'])
            elif file_type == 'forecast_data' and '所属年月' in data.columns:
                data['所属年月'] = pd.to_datetime(data['所属年月'])
            elif file_type == 'sales_target' and '指标年月' in data.columns:
                data['指标年月'] = pd.to_datetime(data['指标年月'])
            elif file_type == 'tt_product_target' and '指标年月' in data.columns:
                data['指标年月'] = pd.to_datetime(data['指标年月'])
            elif file_type == 'customer_target' and '月份' in data.columns:
                data['月份'] = pd.to_datetime(data['月份'])
            elif file_type == 'promotion_data':
        ...
        if file_path.endswith('.xlsx') or file_path.endswith('.parquet'):
            return pd.DataFrame()
        else:
            return []
...
# 加载所有数据源或部分数据源
@st.cache_data(ttl=604800)  # 缓存一周 (7*24*60*60=604800秒)
def load_all_data(data_paths, required_data_types=None):
    """
    选择性加载数据源

    参数:
        data_paths (dict): 包含所有数据文件路径的字典
        required_data_types (list): 需要加载的数据类型列表，如果为None则加载全部
    ...
    if required_data_types is None:
        # 如果没有指定，则加载所有数据类型
        data_types_to_load = list(data_paths.keys())
    else:
        # 只加载指定的数据类型
        data_types_to_load = required_data_types

    # 只加载需要的数据
    for data_type in data_types_to_load:
        if data_type in data_paths:
            data[data_type] = load_data_file(data_paths[data_type], data_type)
...
# 加载特定页面需要的数据
# 加载特定页面需要的数据
def load_page_data(data_paths, page_name, current_data=None):
    """
    根据页面名称加载所需数据

    参数:
        data_paths (dict): 数据文件路径
        page_name (str): 页面名称
        current_data (dict): 当前已加载的数据
    ...
    if page_name == "总览":
        required_data = ['sales_data', 'inventory_data', 'sales_target']
    elif page_name == "销售分析":
        required_data = ['sales_data', 'sales_target', 'customer_relations']
    elif page_name == "客户分析":
        required_data = ['sales_data', 'customer_relations', 'customer_target']
    elif page_name == "产品分析":
        required_data = ['sales_data', 'product_codes', 'tt_product_target']
    elif page_name == "库存分析":
        required_data = ['inventory_data', 'monthly_inventory', 'sales_data']
    elif page_name == "物料分析":
        required_data = ['inventory_data', 'sales_data']
    elif page_name == "新品分析":
        required_data = ['sales_data', 'new_product_codes', 'forecast_data']

    # 找出尚未加载的数据类型
    missing_data_types = [dt for dt in required_data if dt not in current_data]

    # 如果有缺失的数据类型，则加载
    if missing_data_types:
        for data_type in missing_data_types:
            if data_type in data_paths:
                current_data[data_type] = load_data_file(data_paths[data_type], data_type)

    # 预处理销售数据 - 分离MT和TT渠道（如果刚刚加载了sales_data）
    if 'sales_data' in missing_data_types and 'sales_data' in current_data and not current_data['sales_data'].empty:
        # 判断订单类型创建渠道字段
        current_data['sales_data']['渠道'] = current_data['sales_data']['订单类型'].apply(
            lambda x: 'TT' if x == '订单-TT产品' else ('MT' if x == '订单-正常产品' else '其他')
        )

        # 过滤销售订单（只包括正常产品和TT产品）
        current_data['sales_orders'] = current_data['sales_data'][
            current_data['sales_data']['订单类型'].isin(['订单-正常产品', '订单-TT产品'])
        ].copy()

        # 过滤费用订单（促销相关订单）
        current_data['expense_orders'] = current_data['sales_data'][
            current_data['sales_data']['订单类型'].isin([
                '陈列激励明细-F1', '促销补差支持-F1',
                '促销搭赠支持-F1', '门店运维激励费用-F3',
                '全国旧日期库存处理-F3'
            ])
